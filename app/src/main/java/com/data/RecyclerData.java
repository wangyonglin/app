...
public class RecyclerData {
    public  static void  load(Context context, RecyclerView recyclerView, List<Webcams> list){
        int spanCount = 3;
    ...
    public static  void  load(Context context, RecyclerView recyclerView, List<Webcams> list, ResultCallback<Webcams> resultCallback){

        final GridLayoutManager layoutManager = new GridLayoutManager(context,2);//定义瀑布流管理器，第一个参数是列数，第二个是方向。
        //layoutManager.setGapStrategy(StaggeredGridLayoutManager.GAP_HANDLING_NONE);//不设置的话，图片闪烁错位，有可能有整列错位的情况。
        recyclerView.setPadding(0, 0, 0, 0);
        recyclerView.setLayoutManager(layoutManager);//设置瀑布流管理器
        RecyclerAdapter adapter = new RecyclerAdapter(list,context);
        adapter.setItemClickListener(new OnMovieItemClickListener() {
            ...
            public void onItemClick(Webcams webcams) {
                resultCallback.onItemClick(webcams);
            }
        });
        recyclerView.setAdapter(adapter);
    }
